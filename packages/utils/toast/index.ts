...
const commonToast = {
  options: {
    ...
    toastLife: 3600, // How long a toast will stay on the screen before fading away
    currentToasts: 0, // Current number of toasts on the screen
    stackedToasts: false, // Whether or not to stack toasts
  },
  toast: function({ title, message, type, location, icon, theme, dismissable, glass, center }: {
    title?: string
    message?: string
    type?: string
    location?: string
    ...
    if (document.getElementById('ctoaster') == null) {
      // toaster doesn't exist, create it
      const toaster = document.createElement('div') as HTMLElement
      toaster.id = 'ctoaster'
      if (location == null) {
        toaster.className = 'ctoaster top-center'
      } else {
        toaster.className = 'ctoaster ' + location
      }
      document.body.appendChild(toaster)

      // Create the toasting rack inside of the toaster
      if (document.getElementById('commonToastRack') == null) {
        const rack = document.createElement('ol')
        rack.id = 'commonToastRack'
        rack.className = 'rack'
        toaster.appendChild(rack)
      }
    } else {
      const toaster = document.getElementById('ctoaster') as HTMLElement
      // check what location the toaster is in
      toaster.classList.forEach(function(item) {
        // remove any location classes from the toaster
        if (
          item.includes('top-center') || item.includes('top-right') || item.includes('top-left') || item.includes('bottom-right')
          || item.includes('bottom-center') || item.includes('bottom-left')
        ) {
      ...
      // const rack = document.getElementById('commonToastRack')
    }

    // Check if there are too many toasts on the screen
    if (commonToast.options.currentToasts >= commonToast.options.maxToasts) {
      // there are too many toasts on the screen, delete the oldest one
      const oldestToast = document.getElementById('commonToastRack')?.firstChild as Node
      document.getElementById('commonToastRack')?.removeChild(oldestToast)
      commonToast.options.currentToasts--
    }

    // Create the toast
    const toast = document.createElement('li')
    commonToast.options.currentToasts++
    toast.className = 'commonToasttoast'
    // if the toast class contains a top or bottom location, add the appropriate class to the toast
    const toaster = document.getElementById('ctoaster') as HTMLElement
    if (toaster.className.includes('top-center') || toaster.className.includes('top-right') || toaster.className.includes('top-left')) {
    ...
    toast.id = 'commonToastToast-' + commonToast.options.currentToasts
    if (type != null) {
    ...
    document.getElementById('commonToastRack')?.appendChild(toast)
    ...
    if (icon != null && icon == true) {
      ...
      if (type != null) {
        ...
        toast.className += ' ' + type
        if (type == 'success') {
          toastIcon.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">'
    ...
    if (dismissable != null && dismissable == true) {
      ...
      toast.addEventListener('click', function() {
        commonToast.despawnToast(toast.id)
      })
    }
    ...
    setTimeout(function() {
      commonToast.despawnToast(toast.id)
    }, commonToast.options.toastLife)
  },
  despawnToast(toastId: string) {
    // fade out the toast and then remove it from the DOM
    const toast = document.getElementById(toastId) as HTMLElement
    ...
    if (toast != null) {
      ...
      setTimeout(function() {
        ...
        if (commonToast.options.currentToasts == 0) {
          const toaster = document.getElementById('ctoaster') as HTMLElement
